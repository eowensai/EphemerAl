...
with st.sidebar:
    ...
    if st.button("New Conversation", icon=":material/refresh:", key="sidebar_new", use_container_width=True, help="Clear all messages and uploaded files to start fresh."):
...
if HAS_DEVICE_DETECTION and device and device.is_mobile:
    ...
    # for easier reach.
    if st.button("New Conversation", icon=":material/refresh:", key="mobile_new", use_container_width=True, help="Clear all messages and uploaded files to start fresh."):
        st.session_state.clear()
        st.rerun()
